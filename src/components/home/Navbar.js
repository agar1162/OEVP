...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 w-full bg-[#003A70]">
      <div className="flex items-center justify-between px-6 md:px-10 h-[94px]">
        {/* Logo */}
        <a href="/" className="flex items-center">
          <img src={logoref} alt="Logo" className="h-[80px] w-auto object-contain" />
        </a>

        {/* Desktop Menu */}
        <div className="hidden lg:flex items-center space-x-6 text-white font-bold text-[17px] font-[Georgia] lg:ml-4 xl:ml-6">
          ...
          >
            <div className="flex items-center gap-[5px]">
              ...
              <img
                src={icon}
                alt="Dropdown"
                className="w-[14px] h-[7px] mt-[1px] transition group-hover:brightness-75"
              />
            </div>
            ...
            {isPartnershipsMenuVisible && (
              <ul
                className="absolute top-full left-0 mt-2 bg-berkeley border-t-2 border-[#FDB515] text-white shadow-lg w-60 z-50"
                onMouseEnter={() => {
                  if (hideTimeout.current) clearTimeout(hideTimeout.current);
                  setPartnershipsMenuVisible(true);
                }}
